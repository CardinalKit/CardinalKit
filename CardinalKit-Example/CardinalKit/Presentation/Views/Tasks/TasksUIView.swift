...
//        }
    ...
    var body: some View {
        VStack {
            Text(presenter.studyTitle)
                .font(.system(size: 25, weight:.bold))
                .foregroundColor(self.color)
                .padding(.top, 10)
            Text(presenter.teamName).font(.system(size: 15, weight:.light))
            Text(presenter.todayDate).font(.system(size: 18, weight: .regular)).padding()
            List {
                ForEach(presenter.itemsSections, id: \.self) { key in
                    Section(header: Text(key)) {
                        ForEach(presenter.items[key]!, id: \.self) { item in
